...
public class DataController {
    ...
    public static ResModel GetBoxDetailInfoByIdAndPosition(@RequestBody BoxInfo rawBoxInfo, HttpServletResponse
        ...
        try {
            ...
            res.data = DateService.GetBoxDetailInfoByIdAndPosition(String.valueOf(rawBoxInfo.id), cellPos);
            res.message = "成功获取数据.";
            res.isSuccess = true;
            return res;
        } catch (Exception ex) {
    ...
    public void getPdfShowFile(@PathVariable String eDocId, HttpServletResponse response) {
        ...
        {   //获取本地路径
            ...
            String path = dateService.getViewPdfName(eDocId);
            //获取文件的完整路径
            String filePath = rootPath + path;
            File file = new File(filePath);
            ResModel res = dateService.getPdfShowFile(eDocId);
            if (file.exists()) {
                // 设置强制下载不打开
